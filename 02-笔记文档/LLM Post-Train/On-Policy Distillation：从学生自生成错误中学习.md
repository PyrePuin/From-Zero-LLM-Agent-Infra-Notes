...
把 OPD 与 RL 结合，是因为二者解决的问题不同。RL 可以直接优化事实性、安全性或人类偏好等外部奖励，但单一标量奖励比较稀疏，也可能使 Student 为追逐奖励而偏离原有能力；OPD 提供逐 Token 的 Teacher 分布监督，相当于在优化奖励时持续为 Student 提供能力锚点。

这种组合的主要好处是：在改善目标奖励的同时尽量保留 Teacher 的任务质量，并且两种训练可以复用同一批 Student Rollout。若目标只是模仿 Teacher，并没有额外奖励需要优化，就没有必要加入 RL。在 XSum 实验中，作者把基于文本蕴含的事实性奖励与 GKD 结合，在摘要质量和事实一致性之间形成由 $`\alpha`$ 控制的权衡。
