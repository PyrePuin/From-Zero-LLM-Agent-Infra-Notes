...
### 4.2 RoPE 的相对性为什么更有利于外推

Sinusoidal PE 和 RoPE 都使用不同频率的正弦、余弦，因此二者在数学上都包含相对位置结构。它们最重要的区别不是“一个有相对性、另一个没有”，而是**相对关系进入 Attention 的方式不同**。

Sinusoidal PE 先把绝对位置向量加到 token embedding：

```math
x_p=e_p+PE(p)
```

它满足 $`PE(p+k)`$ 可以由 $`PE(p)`$ 经过一个只依赖 $`k`$ 的线性变换得到，而且原始位置向量的点积也只依赖相对距离。因此，Sinusoidal PE 确实具有相对性；但模型需要在内容与位置相加后的表示中，自己学会提取这种关系。

RoPE 则直接让旋转后的 Q/K 点积满足：
...
如果 Query 和 Key 同时平移 $`c`$ 个位置，它们的相对位移保持不变：

```math
(s+c)-(p+c)=s-p
```

所以，同一种相对位置模式可以在序列的不同绝对位置复用。例如，模型在训练时学到“关注前面 3 个 token”，无论当前位置是 10 还是 1000，对应的相对位移始终是 $`-3`$。这种平移一致性直接存在于 Attention 点积公式中，因此通常比让模型从两个绝对位置向量中自行推导更自然。

不过，这只说明 RoPE 具有更直接的相对位置归纳偏置，**不代表它自动拥有可靠的长度外推能力**。训练长度从 $`L`$ 扩展到更长序列后，模型仍会遇到训练中没有见过的相对距离、旋转角和 Attention 模式。

因此，更准确的对比是：

| 方法 | 相对关系从哪里产生 | 外推时的主要问题 |
| --- | --- | --- |
| Sinusoidal PE | 从两个绝对位置向量及 Q/K 投影中间接学习 | 出现未见过的绝对位置及相位组合 |
| RoPE | 旋转后的 Q/K 点积天然依赖相对位移 | 出现未见过的相对距离及旋转相位 |

### 4.3 RoPE 的主要缺陷

#### 4.3.1 原始 RoPE 仍然存在长度分布外问题

RoPE 可以为任意位置计算旋转角，因此不会像有限的可学习绝对位置表那样直接越界。但模型只在有限上下文中训练时，只学过有限范围的相对距离和相位组合。推理长度远超训练长度后，Attention logit 的数值分布和注意力模式都可能变化，性能仍会下降。

#### 4.3.2 周期性会带来相位绕回与振荡

RoPE 使用正弦、余弦表示旋转，相位具有周期性。同一频率下，角度增加完整周期后会回到相同方向。虽然多组频率共同作用，使完整位置表示不容易在短范围内完全重复，但在很长距离上仍可能出现相位绕回、相似度振荡和位置区分能力下降。

这也意味着 RoPE 的距离影响不是像 ALiBi 那样严格单调。ALiBi 的距离惩罚会随距离线性增大；RoPE 的相对位置项由多组正弦、余弦共同决定，某些更远位置可能因为相位重新接近而得到较高的相对位置匹配。

#### 4.3.3 不同频率难以用同一种缩放同时照顾

高频维度负责细粒度的局部位置变化，低频维度负责较长尺度的位置变化。如果简单地把所有位置都除以同一个扩展倍数，虽然能把长序列压回训练范围，却也会压缩近距离之间的角度差，损伤模型原来学到的局部位置分辨率。

反过来，如果完全保留原始频率，局部能力受到的影响较小，但长距离相位又会进入明显的分布外区域。这就是 Position Interpolation、NTK-aware Scaling 和 YaRN 需要在“保留短距离能力”与“容纳更长距离”之间折中的原因。

#### 4.3.4 相对点积不提供显式的绝对位置锚点

RoPE 使用绝对位置分别旋转 Q 和 K，但旋转后的点积只保留相对位移 $`s-p`$。因此，Attention 的位置项擅长表达“相距多少”，却不会直接告诉模型“当前是第几个 token”或“距离序列末尾还有多远”。

模型仍可能通过 Causal mask、BOS/EOS、序列边界和内容间接推断绝对位置，所以这不是“完全学不到绝对位置”，而是 RoPE 的 Attention 公式没有显式提供绝对坐标。
