...
还要纠正一个容易出现的误解：Sinusoidal PE 和 RoPE 各频率维度的正弦、余弦振幅都没有随维度变大；变化的是频率和波长。高频维度旋转得快，低频维度旋转得慢。训练范围只覆盖了每个频率圆周上的有限相位区间，超出这个区间后，模型未必能延续训练时学到的使用方式。
...
#### 4.3.5 长上下文扩展依赖额外配置与验证

修改 RoPE base 或加入 scaling 并不能单独保证长上下文能力。训练数据必须包含足够的长序列，训练与推理需要使用一致的频率配置，推理框架还要正确处理 KV Cache；最终也需要通过长距离检索、困惑度和实际任务评测验证。

> [!warning]
> RoPE 的优势是相对位置关系进入 Q/K 点积更自然，而不是“从此没有外推问题”。原始 RoPE 仍受训练距离范围、周期相位和频率缩放取舍限制；这正是后续 RoPE scaling 方法要解决的问题。
