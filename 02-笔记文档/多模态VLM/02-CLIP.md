...
要理解这两条支路怎样汇合，可以先分别看它们如何把原始输入压缩成一个全局特征，再看投影层如何消除两侧的维度差异。
...
至此，图片已经被压缩成一个能够代表全局视觉语义的向量。文本侧需要完成同样的工作，只是输入形式和全局表示的取法不同。
...
现在两侧都得到了全局特征，但 $`h_i^I`$ 与 $`h_i^T`$ 可能处在不同维度、不同特征空间中，还不能直接计算语义相似度，因此需要最后一步线性投影。
...
经过投影，一对图片和文本终于都变成了 $`D`$ 维向量。不过，CLIP 的训练单位不是孤立的一对样本，而是整个 Batch；下一步要把 $`N`$ 对样本放在一起，形成可用于对比学习的张量。
...
此时两侧已经各有一个 $`N\times D`$ 矩阵，但直接点积仍会受到向量长度影响。为了让分数主要反映语义方向，计算相似度前还要逐向量归一化。
...
归一化完成后，每个点积都可以解释为余弦相似度。接下来只需把所有图像向量与所有文本向量一次性做矩阵乘法，就能同时得到 Batch 内的全部图文匹配分数。
...
相似度矩阵把对比学习转化成了一个更熟悉的问题：对每一行或每一列做分类。下面先让图片从所有文本中选择正确文本，再反过来让文本选择正确图片。
...
两个方向使用同一张相似度矩阵，只是观察方向不同。先从图片作为查询的行方向开始。
...
按行计算后，模型已经学会“给定图片寻找文本”，但这并不直接约束“给定文本寻找图片”。为了让共享空间中的匹配关系双向成立，还需要计算转置方向的损失。
...
至此，同一张相似度矩阵产生了两个互补的标量损失。最后一步不是继续做一次 Softmax，而是将这两个独立目标合并。
...
取平均后得到一次训练迭代最终优化的标量目标。为了理解这个标量为什么真的能“拉近正样本、推远负样本”，还需要沿着交叉熵的梯度继续向前追踪。
...
上面的梯度解释了相似度应该向哪个方向变化，但变化力度还受到相似度缩放控制。这个缩放就是 CLIP 中可学习的 Temperature 或 Logit Scale。
...
到这里，前向计算和损失函数中的全部组成部分都已经明确。接下来把梯度沿计算图继续传回，便能看到一次优化步骤究竟更新哪些参数。
...
概念链路已经完整，最后可以用一段伪代码把编码、投影、归一化、矩阵乘法、双向损失和参数更新压缩到同一次训练迭代中。
...
经过反复训练，两个 Encoder 学到的向量已经可以跨模态比较。接下来的 Zero-shot 分类不再更新这些参数，而是把候选类别文本编码成一个临时分类器来复用这种对齐能力。

## 4. Zero-shot 分类

Zero-shot 分类不是让 CLIP 自动生成一个类别名称，而是提前给出候选类别，让模型选择最匹配的文本。

它可以拆成两个连续步骤：先用候选类别文本构造分类器，再让新图片与这些类别向量逐一比较。
...
文本侧的临时分类器构造完成后，类别向量可以提前缓存。真正处理每张待分类图片时，只需运行图像侧并计算它与这些类别向量的相似度。
...
整个预测过程只执行前向计算，没有交叉熵、反向传播或参数更新。理解这一点后，就可以更准确地说明这里的“零样本”究竟省略了什么。
...
因此，Zero-shot 省略的是目标任务的标注训练和微调，而不是候选类别本身。不过，同一个类别可以有多种自然语言写法，Prompt 的措辞仍会影响最终类别向量。
...
到这里，Zero-shot 的分类器构造和图片预测过程都已完成。最后用一张表把它与第 3 章的对比预训练并排比较，可以避免把两个阶段的输入和行为混在一起。
...
这种复用方式带来了很强的任务迁移能力，但全局向量匹配也决定了 CLIP 并不能覆盖所有视觉语言任务，下面继续看它的能力边界。
...
这些限制并没有削弱 CLIP 作为图文对齐基础模型的价值，反而解释了后续 VLM 为什么要保留更多局部视觉 Token，并在对齐模块之后继续接入语言模型。
...
明确 CLIP 在这条路线中的位置后，最后再集中清理几个最容易混淆的概念，避免把对比学习模型与生成式 VLM 混为一谈。
...
如果上表中的边界已经清楚，就可以用下面的问题检查自己是否真正串起了训练、推理和模型定位三条主线。
...
这些问题分别覆盖编码、对比损失、参数更新和 Zero-shot 推理。能够脱离公式逐一回答后，再用最后的主链路做一次整体压缩。
