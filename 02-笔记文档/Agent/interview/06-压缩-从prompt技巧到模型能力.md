...
> 通用模型直接做 compaction 是有分布缺陷的，头部产品（Codex 的专用压缩接口）已经把这件事下沉成了模型能力。
> 内含一个可直接讲的自研段子：后训练一个类似 Codex 压缩接口的模型（白马二）。
> 代码锚点仍为 [`core/memory.py`](https://github.com/lasywolf/Learn-OpenClaw/blob/main/core/memory.py)。
...
- *白马二最大的坑是什么？* → 数据构造。「好的 checkpoint 长什么样」没有现成的 ground truth——人标的 checkpoint 未必是模型续跑最好用的。我们的解法是**用下游成功率回采**：同一条轨迹让模型写多版 checkpoint，各自接着跑任务，哪个续跑成功率高就留哪个当正例。本质上是把 eval 变成了数据飞轮（和 06 的思想一脉相承）。
