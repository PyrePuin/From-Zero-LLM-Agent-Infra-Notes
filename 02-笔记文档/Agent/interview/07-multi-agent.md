...
当然，得把话说精确：失败的是 A2A 这种**跨进程、跨组织的对等协议型 multi-agent**，不是 multi-agent 本身——Claude Code 的 agent teams 就是 multi-agent，而且效果拔群（Anthropic 拿一队并行 Claude 写出了 C 编译器）。区分成败的判据不是「agent 的数量」，而是**信任域和基础设施是否共享**：teams 里所有 agent 跑在同一套原语上——git worktree 隔离代码、tmux 隔离会话、CI 做汇合仲裁——协作成本被现成的基础设施压到了最低；而 A2A 设想的是跨越信任域的协作，中间态交换的成本没有任何基础设施兜底，自然不成立。所以一句话：**multi-agent 成立的条件是把协作成本工程化地压下去，A2A 想用协议把这个问题绕过去，绕不过去**。
