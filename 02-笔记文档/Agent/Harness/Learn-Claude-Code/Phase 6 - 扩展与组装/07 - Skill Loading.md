...
1. **两级分工**：catalog 和 SKILL.md 内容分别注入到哪里？代价差多少？（→ [核心抽象](#核心抽象)）
2. **注册中心**：`SKILL_REGISTRY` 什么时候填充？`load_skill` 为什么 O(1)？（→ [核心抽象](#skill-registry-与-load_skill)）
3. **Manifest Pattern**：SKILL.md 不是可执行入口——scripts 怎么被跑？（→ [核心抽象](#manifest-patternskillmd-是清单不是入口)）
4. **frontmatter 极简**：s07 只解析哪两个字段？为什么不用 PyYAML 解全部？（→ [核心抽象](#parse_frontmatter-极简解析)）
5. **防路径遍历**：`load_skill` 只接收 name 而非 path，安全在哪？（→ [设计要点](#5-防路径遍历)）

**可以略读/跳过**：CC 的 7 个 skill 加载来源、丰富的 frontmatter 字段（`when_to_use` / `paths` / `context: fork`）——这些是 CC 产品级特性，理解两级加载概念不需要。
...
| `SKILL_REGISTRY` | dict，启动时填充，存 name + description + content | ⭐⭐⭐ |
| `_scan_skills()` | 启动时扫描目录，解析 frontmatter，填充注册表 | ⭐⭐ |
| `_parse_frontmatter()` | 解析 SKILL.md 的 `---yaml---` 块（只取 name/description） | ⭐ |
| `list_skills()` | 从注册表生成 catalog 字符串 | ⭐⭐ |
| `build_system()` | 把 catalog 注入 SYSTEM prompt | ⭐⭐⭐ |
| `load_skill(name)` 工具 | 返回完整 SKILL.md 内容（按 name 查 registry） | ⭐⭐⭐ |

## 演进与动机

### 反例：全塞 SYSTEM 等于永远为不需要的东西付费
...
更糟的是：**SYSTEM 每轮都要带**。这是 API 计费的主要部分（输入 token 比 output 贵）。等于每个工具回合都为无关文档付费。**项目越成熟，规范文档越多**——React 规范、SQL 规范、API 规范、Git 提交规范、Code Review 清单、部署流程……塞 SYSTEM 不可持续。

### 解法核心：两级加载

需要一种"**有但不用就当不存在**"的机制——文档存在磁盘上，模型知道有，但只在需要时才读到。再加一层："让 Agent 主动判断现在需要哪份文档"。如果只让模型用 `read_file("docs/sql-style.md")` 自己读，问题在于：**模型怎么知道这个文件存在**？它没法列目录（除非又调 bash + ls，又消耗 token）。

s07 的解法：把"有哪些技能"放进 SYSTEM（一行一条，便宜），让模型看到名字和描述后**自己决定**调 `load_skill`。

### 产品需求收紧叙事

这是 **Two-Tier Knowledge Loading + Manifest Pattern**。和 [[09 - Memory]] 的"索引 + 内容"是同构的——便宜层做"高频访问、低信息量"（目录），贵层做"低频访问、高信息量"（完整内容）。操作系统 L1/L2/L3 缓存、CPU 缓存、CDN 都是这个模式。
...
### SKILL_REGISTRY 与 load_skill

`SKILL_REGISTRY` 是**启动时一次性填充**的 dict。运行时 `load_skill(name)` 只是 dict 查表，不走文件系统：
...
好处：**O(1) 查找** + **无路径遍历风险**（只接收 name） + **可枚举**（一次生成完整目录）。代价：技能文件**运行中修改**不会被读到（要重启）。但技能是配置类资源，运行中改的场景极少。

### _parse_frontmatter 极简解析

s07 的 `_parse_frontmatter` 只看 `name` 和 `description`，其他字段忽略。这避免了 YAML 解析的复杂性（教学版手写解析，不用 PyYAML）。生产实现会用 PyYAML 或类似的库，把所有字段都解析出来。

## 整体架构图

两级加载的完整数据流：启动时一次性扫描填充 registry，运行时 catalog 常驻 SYSTEM（便宜层），模型按需调 `load_skill` 把 SKILL.md 注入 tool_result（贵层）。
...
flowchart TB
    subgraph Startup["启动阶段"]
        Scan["<b>_scan_skills</b><br/>扫 skills/ 目录<br/>解析 frontmatter<br/>填充 SKILL_REGISTRY"]
    end

    subgraph Tier1["第 1 级：便宜层<br/>常驻 SYSTEM"]
        Catalog["<b>技能目录</b><br/>每条 ≤ 100 token<br/>name + description<br/>每轮 API 都带"]
    end

    Decide{"<b>模型判断</b><br/>需要哪个技能?"}
    Load["<b>load_skill(name)</b><br/>从 registry 取 content"]

    subgraph Tier2["第 2 级：贵层<br/>按需 tool_result"]
        SK1["<b>SKILL.md 完整内容</b><br/>~2000 token/技能<br/>只在该技能被加载时带"]
    end
    ...
    class Scan,Load mech
    class Catalog,SK1 data
...
剥掉 frontmatter 解析细节后，s07 的核心抽象层只有这么多代码。这是你真正需要记住的部分——多来源加载、rich frontmatter 字段等 CC 产品级特性都可以忘掉。
...
# === 2. 启动时扫描 ===
def _parse_frontmatter(text: str) -> tuple[dict, str]:
    """解析 SKILL.md 的 ---yaml--- 块（极简：只用 PyYAML）。"""
    if not text.startswith("---"):
        return {}, text
    parts = text.split("---", 2)
    if len(parts) < 3:
...
def _scan_skills():
    """扫 skills/ 目录，每个子目录的 SKILL.md → 注册到 SKILL_REGISTRY。"""
    if not SKILLS_DIR.exists():
        return
    for d in sorted(SKILLS_DIR.iterdir()):
        if not d.is_dir():
        ...
        if manifest.exists():
            raw = manifest.read_text()
            meta, body = _parse_frontmatter(raw)
            # 极简：只取 name + description（其他字段忽略）
            name = meta.get("name", d.name)
            desc = meta.get("description", raw.split("\n")[0].lstrip("#").strip())
            SKILL_REGISTRY[name] = {"name": name, "description": desc, "content": raw}

_scan_skills()   # 模块加载时执行一次

# === 3. 注入 catalog 到 SYSTEM（便宜层）===
def list_skills() -> str:
    if not SKILL_REGISTRY:
        return "(no skills found)"
...
SYSTEM = build_system()   # 启动时构建一次，整个进程内不变

# === 4. load_skill 工具（贵层，按需）===
def load_skill(name: str) -> str:
    """O(1) dict lookup，不走文件系统，无路径遍历风险。"""
    skill = SKILL_REGISTRY.get(name)
    if not skill:
        return f"Skill not found: {name}"
    return skill["content"]
...
# === 5. agent_loop 复用 s01-s06 的标准循环 ===
# load_skill 不需要特殊处理——LLM 调它时走标准 dispatch
# 返回的 SKILL.md 内容进 tool_result，下一轮 LLM 能看到并据此行动
```

**这 5 块是 s07 的全部抽象层**。注意 SYSTEM 在 s07 是**真正静态**的（启动扫一次就固定），这跟 s10 每轮重算 system 不同。下一节 s08 会在长会话里把已加载的 SKILL.md 内容压缩掉。
