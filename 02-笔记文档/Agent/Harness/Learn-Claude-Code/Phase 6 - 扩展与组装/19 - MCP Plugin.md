...
1. **三方架构**：Agent / MCPClient / MCP Server 各自的职责边界？（→ [核心抽象](#核心抽象)）
2. **协议契约**：`tools/list` 和 `tools/call` 分别做什么？教学版的 `register` / `call_tool` 怎么对应？（→ [核心抽象](#协议语义toolslist--toolscall)）
3. **命名空间**：`mcp__{server}__{tool}` 三段式前缀解决什么冲突？（→ [核心抽象](#命名空间mcp__server__tool)）
4. **异步状态机**：为什么 `connect_mcp` 不能直接改 tools 数组？什么时点工具才真正进 agent？（→ [核心抽象](#标记变更--下次循环生效模式)）
5. **agent_loop 改动**：s19 为何去掉 s10 的 prompt cache？connect_mcp 触发即时重建在干什么？（→ [对 agent_loop 的影响](#对-agent_loop-的影响)）

**可以略读/跳过**：CC 的 6 种 transport（stdio/sse/http/ws/sse-ide/sdk）、OAuth + PKCE 认证流、Channel notification 反向通知、5 层配置优先级。这些是 CC 产品级工程特性，理解 MCP 概念不需要。

## 这一步加了什么

| 新增 | 作用 | 重点? |
|---|---|---|
| `MCPClient` 类 | Agent 端的 server 代理，持有 tools 定义和 handlers | ⭐⭐⭐ |
| `mcp_clients` 全局 dict | 已连接 server 的登记表——连接状态唯一真相 | ⭐⭐⭐ |
| `connect_mcp(name)` | Lead 工具：连接 server（登记字典，不改 tools） | ⭐⭐⭐ |
| `normalize_mcp_name(name)` | 名称规范化：非 `[a-zA-Z0-9_-]` 替换为 `_` | ⭐⭐ |
| `assemble_tool_pool()` | 核心合并函数：builtin + MCP 工具 → 统一 (tools, handlers) | ⭐⭐⭐ |
| `assemble_system_prompt` 升级 | 加 "Connected MCP servers: ..." 列表 | ⭐⭐ |
| 工具描述 readOnly/destructive 标注 | 文本提示 LLM 谨慎（CC 升级成结构化 annotations） | ⭐ |
| `agent_loop` 改动 | 每轮开头 `assemble_tool_pool` + 去掉 s10 prompt cache + connect_mcp 后即时重建 | ⭐⭐⭐ |

## 演进与动机

### 反例 1：手写工具不能扩展

s01-s18 的所有工具都是**项目内置**的——bash / read / write / task / worktree。如果你要接 Jira API、自建部署系统、Notion 知识库、GitHub / Slack / Linear 集成——**每个都要改项目代码**：加 TOOLS 数组、写 handler、加 input_schema、加错误处理。这违反"开放-封闭"原则。

### 反例 2：每个外部服务自己设计接入方式

服务 A 用 REST、B 用 gRPC、C 用 GraphQL、D 用 CLI——Agent 要为每个服务学一套接入代码，**集成成本随服务数线性增长**。

### 反例 3：两个 server 都有 `search` 工具

docs.search 和 slack.search 直接合并会冲突，后者覆盖前者。

### 解法核心：标准协议 + 命名空间 + 动态工具表

s19 的解法是**三层叠加**：

1. **标准协议**——MCP 用 `tools/list`（声明能力）+ `tools/call`（执行调用）。服务实现这两个 RPC 就能接入，Agent 不关心服务怎么实现、用什么语言、跑在哪。
2. **命名空间隔离**——`mcp__{server}__{tool}` 前缀避免冲突：`mcp__docs__search` vs `mcp__slack__search`，零冲突。
3. **动态工具表**——connect 一个 server 后工具立即增加；disconnect 后减少。agent_loop 通过"每轮重建 + connect_mcp 触发即时重建"处理工具表变化。

### 产品需求收紧叙事

教学版用 Python 函数 mock 两个 server（docs / deploy），聚焦**协议语义**而非**传输工程**——真实 MCP 用 stdio / http / sse / ws 通信，配 OAuth + PKCE + 重试，但概念骨架完全一致。这是 Phase 6 的第一课：**Agent 第一次能"扩展"而不是"被扩展"**。

## 核心抽象

### 三方架构：Agent ↔ MCPClient ↔ MCP Server

教学版的 MCPClient 是"模拟"——server 实现就在同进程的 Python 函数里。真实 MCP 协议下，MCPClient 通过**网络 / 进程间通信**（stdio / http / sse / ws）跟外部 server 对话。
...
| 方法 | 方向 | 用途 | 教学版对应 |
|---|---|---|---|
| `tools/list` | Agent → Server | "你有哪些工具？给我 schema" | `MCPClient.register()` |
| `tools/call` | Agent → Server | "执行这个工具，参数是 ..." | `MCPClient.call_tool()` |

简化但语义一致——`register` 是 `tools/list` 的"一次性灌入版"，`call_tool` 走完 `handlers.get(name)` → `handler(**args)` 模拟远程调用 + 错误兜底。
...
跟 s14 cron 的"入队 + queue processor 消费"、s16 协议的"pending_requests 状态机"是同构模式——**异步状态机**：dispatch 阶段是"执行当前决策"，不能改"决策依据"。改依据要等下一轮。
...
flowchart LR
    subgraph Agent["Agent (Lead)"]
        A1["<b>agent_loop</b>"]
        A2["<b>assemble_tool_pool</b><br/>合并 builtin + MCP"]
    end

    subgraph Client["MCPClient 代理（s19 新增）"]
        C1["<b>register</b><br/>模拟 tools/list"]
        C2["<b>call_tool</b><br/>模拟 tools/call"]
        MC["<b>mcp_clients</b> dict<br/>已连接 server 登记"]
    end

    subgraph Server["外部 Server"]
        S1["<b>docs server</b><br/>search / get_version"]
        S2["<b>deploy server</b><br/>trigger / status"]
    end

    A1 -->|"调 connect_mcp"| A2
    A2 -->|"读 mcp_clients"| MC
    MC --> C1
    A1 -->|"LLM 调 mcp__docs__search"| C2
...
flowchart TB
    subgraph Static["静态定义"]
        B1["<b>BUILTIN_TOOLS</b><br/>BUILTIN_HANDLERS"]
        MS["<b>MOCK_SERVERS</b><br/>{name: factory}"]
    end

    subgraph Runtime["运行时状态"]
        MC["<b>mcp_clients</b><br/>{name: MCPClient}"]
    end

    subgraph MCP["MCP 核心（s19 新增）"]
        M1["<b>MCPClient</b><br/>register + call_tool"]
        M2["<b>connect_mcp</b><br/>Lead 工具"]
        M3["<b>assemble_tool_pool</b><br/>合并 builtin + MCP"]
        M4["<b>normalize_mcp_name</b><br/>名称规范化"]
    end
    ...
    subgraph Loop["agent_loop"]
        ...
        L3["dispatch 工具"]
        L4["检测 connect_mcp 后<br/>即时重建"]
    end

    subgraph Lead["Lead 工具"]
        T1["<b>connect_mcp</b><br/>(BUILTIN 之一)"]
        T2["<b>bash / read / ...</b>"]
        T3["<b>mcp__docs__search 等</b><br/>(动态生成)"]
    end
...
flowchart TB
    subgraph Main["主线程"]
        Lead["<b>Lead agent_loop</b><br/>动态 tools（含 MCP）"]
    end

    subgraph Teammates["Teammate daemon threads"]
        T1["<b>Alice</b><br/>固定 8 工具<br/>不能用 MCP"]
        T2["<b>Bob</b><br/>固定 8 工具<br/>不能用 MCP"]
    end

    subgraph Shared["共享状态"]
        MC["<b>mcp_clients</b><br/>(只有 Lead 写)"]
    end
...
剥掉 mock server 实现和 WS / JSON-RPC 样板后，s19 的抽象层只有这么多代码。详细走读在 [实现对照](#实现对照s19codepy)，这里只看核心契约：
...
# === 2. MCPClient —— Agent 端的 server 代理 ===
class MCPClient:
    def __init__(self, name: str):
        self.name = name
        self.tools: list[dict] = []                # tools/list 响应（schema）
        self._handlers: dict[str, Callable] = {}   # tool_name → 实现

    def register(self, tool_defs, handlers):       # 模拟 tools/list
        self.tools = tool_defs
        self._handlers = handlers
...
# === 4. connect_mcp —— Lead 工具：只登记字典，不改 tools ===
def connect_mcp(name: str) -> str:
    ...
    mcp_clients[name] = factory()                  # 登记（工具真正生效在下一轮）
    return f"Connected to MCP server '{name}'. Discovered N tools: ..."

# === 5. assemble_tool_pool —— 核心合并：builtin + MCP ===
def assemble_tool_pool() -> tuple[list[dict], dict]:
    tools = list(BUILTIN_TOOLS)
    handlers = dict(BUILTIN_HANDLERS)
    for server_name, mcp_client in mcp_clients.items():
        ...
        for tool_def in mcp_client.tools:
            safe_tool = normalize_mcp_name(tool_def["name"])
            prefixed = f"mcp__{safe_server}__{safe_tool}"   # 三段式命名空间
            tools.append({
                "name": prefixed,
                "description": tool_def.get("description", ""),
                "input_schema": tool_def.get("inputSchema", {}),
...
# === 6. agent_loop —— 每轮重建 + connect_mcp 后即时重建 ===
def agent_loop(messages, context):
    tools, handlers = assemble_tool_pool()         # ← 每轮开头组装（去掉 s10 缓存）
    system = assemble_system_prompt(context)
    while True:
        response = client.messages.create(..., tools=tools, ...)
        messages.append({"role": "assistant", "content": response.content})
...
**这 6 块是 s19 的全部抽象层**。下一节 s20 把 s01-s19 的所有机制装回同一个 `agent_loop`——`assemble_tool_pool` 是其中之一。
