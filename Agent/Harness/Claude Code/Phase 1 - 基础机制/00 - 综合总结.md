...
```
s01: 一个能调 bash 的循环        ← 最小骨架
  │
  └→ s02: 工具抽象成一张表        ← 从"硬编码 bash"到"派发表"
        │
        └→ s03: 工具执行前加闸门  ← 拦截危险操作
              │
              └→ s04: 把闸门/日志/截断抽出去  ← 循环回归纯净，所有副作用挂到 hook 上
```
...
```
       ...
       │    ┌──────────────────────────────────┐  │
       │    │  PreToolUse hooks (s04)          │  │
       │    │   ├── permission_hook (s03)      │  │
       │    │   ├── log_hook                   │  │
       │    │   └── ...                        │  │
       │    └──────────────────────────────────┘  │
       │           ↓ allow                        │
       │    handler = TOOL_HANDLERS[b.name] (s02) │
       │    output = handler(**b.input)           │
       │           ↓                              │
       │    ┌──────────────────────────────────┐  │
       │    │  PostToolUse hooks (s04)         │  │
       │    │   ├── large_output_hook          │  │
       │    │   └── ...                        │  │
       │    └──────────────────────────────────┘  │
       └──────────────────────────────────────────┘
