...
1. **归位本质**：s20 没有新机制，每个零件来自前面哪一课？挂在循环的哪个位置？（→ [这一步加了什么](#这一步加了什么)）
2. **流水线工位**：LLM 调用前的注入序列是什么（cron → bg → todo reminder → prepare_context → assemble_tool_pool）？为什么按这个顺序？（→ [核心抽象](#核心抽象)）
3. **tool_use 检测**：为什么 s20 用 `has_tool_use(response.content)` 而非 `stop_reason == "tool_use"`？（→ [核心抽象](#has_tool_use--stop_reason-的可靠性)）
4. **agent_lock 串行化**：CLI / cron / teammate 同时触发时怎么互斥？（→ [核心抽象](#agent_lock-与多线程并行)）
5. **prompt cache 取舍**：s20 为何放弃 s10 的 cache？正确性 vs 性能怎么权衡？（→ [设计要点](#1-每轮重建-system-prompt-和-tool-pool)）

**可以略读/跳过**：CC 的产品化深度对比表（数十个内置工具 / IDE 集成 / 企业策略）、27 工具的完整列表——这些是 CC 工程细节，理解"归位"概念只需要骨架。
...
s05 单独讲 todo 时，循环里没有 hooks、没有压缩、没有 MCP——你能看清楚 todo 怎么管"会话内计划"，但看不到它和 hooks 的协同。s13 单独讲 background 时，没有 cron、没有团队——你能看清楚 placeholder 模式，但看不到 background 和 task_notification 在完整循环里的位置。
...
**s20 的存在是为了证明这些机制可以共存且不冲突**。教学版用 27 个工具 + 4 个 hook 点 + 一个 agent_loop 把它们全部装下。每个零件有自己的位置，互不打架——"归位"本身就是 s20 的机制。

命名同构：**归位 / integration**（按循环时间轴重新组织）+ **流水线 / pipeline**（prepare_context 四级压缩是典型 pipeline）+ **每个工位单一职责 / SRP**（PreToolUse 只管拦截、PostToolUse 只管后处理、dispatcher 只管派发）+ **生命周期 / lifecycle**（Task / Teammate / CronJob 各自状态机）。

## 核心抽象

### 流水线：每轮 agent_loop 的工位序列
...
flowchart TD
    Start([本轮开始]) --> Cron["<b>consume_cron_queue</b><br/>定时任务注入 messages"]
    Cron --> BG["<b>inject_background_notifications</b><br/>后台完成结果注入 messages"]
    BG --> Todo["<b>rounds_since_todo ≥ 3?</b><br/>注入 reminder"]
    Todo --> Prep["<b>prepare_context</b><br/>四级压缩管线"]
    Prep --> Ctx["<b>update_context + assemble_tool_pool</b><br/>合并工具池"]
    Ctx --> Call["<b>call_llm</b><br/>with_retry 包错误恢复"]
    Call --> Check{"<b>stop_reason?</b>"}
    Check -->|"max_tokens"| Esc["<b>升级 / continuation</b>"]
    Check -->|"正常"| Append["<b>messages.append assistant</b>"]
    Append --> HasTool{"<b>has_tool_use?</b>"}
    HasTool -->|"否"| StopH["<b>trigger_hooks Stop</b><br/>return"]
    HasTool -->|"是"| Loop["<b>遍历 tool_use blocks</b>"]
    Loop --> P1["<b>PreToolUse hooks</b><br/>+ permission"]
    P1 --> BG2{"<b>should_run_background?</b>"}
    BG2 -->|"是"| BGDisp["<b>start_background_task</b><br/>placeholder"]
    BG2 -->|"否"| Exec["<b>call_tool_handler</b>"]
    Exec --> P2["<b>PostToolUse hooks</b>"]
    P2 --> Next{"<b>更多 block?</b>"}
    Next -->|"是"| Loop
    Next -->|"否"| Build["<b>build_user_content</b><br/>tool_results + bg"]
    Build --> Start
    Esc --> Start

    class Start,StopH terminal
    class Cron,BG,Todo,Prep,Ctx,Append,Loop,P1,P2,Exec,BGDisp,Build,Esc mech
    class Call core
    class Check,HasTool,BG2,Next decision
...
### has_tool_use & stop_reason 的可靠性

s20 把 s01 的 `if response.stop_reason == "tool_use"` 换成 `if not has_tool_use(response.content): return`——以**实际 block 类型**为信号，不是 stop_reason 字段。
...
**原因**：模型有时返回 `stop_reason="end_turn"` 但内容里有 tool_use block（罕见但存在，例如 max_tokens 切到 tool_use 块中间时）；以 block 为准更可靠。CC 也这么做。**工程纪律**：真相在 messages 里，不在 metadata 字段里。
...
| 线程 | 数量 | 职责 | 同步机制 |
|---|---|---|---|
| **CLI / main** | 1 | `input()` 读用户、调 `agent_loop`、Lead inbox 轮询 | `agent_lock`（互斥进入 agent_loop） |
| **cron_scheduler_loop** | 1（daemon） | 每秒检查 cron jobs，命中 push 到 `cron_queue` | `cron_lock` |
...
flowchart LR
    subgraph CLI["main loop (CLI thread)"]
        Input["<b>input()</b>"]
        Turn["<b>history.append</b>"]
        Lock["<b>agent_lock</b>"]
    end

    subgraph Loop["agent_loop (per turn)"]
        Cron["<b>consume_cron_queue</b>"]
        BG["<b>inject_bg_notifications</b>"]
        Prep["<b>prepare_context</b>"]
        Pool["<b>assemble_tool_pool</b>"]
        Call["<b>call_llm + with_retry</b>"]
        Disp["<b>dispatch tool_use</b>"]
        Build["<b>build_user_content</b>"]
    end

    subgraph Hooks["<b>trigger_hooks</b>"]
        UPS["UserPromptSubmit"]
        PRE["PreToolUse<br/>(permission + log)"]
        POST["PostToolUse"]
        STP["Stop"]
    end

    subgraph Workers["daemon threads"]
        CronThread["<b>cron_scheduler</b>"]
        BGThread["<b>bg worker</b> per task"]
        Mate["<b>teammate_thread</b> × N"]
    end

    subgraph State["持久化"]
        Tasks[("<b>.tasks/*.json</b>")]
        Mem[("<b>.memory/</b>")]
        WT[("<b>.worktrees/</b>")]
        CronJson[("<b>.cron_jobs.json</b>")]
        Skills[("<b>skills/</b>")]
    end
    ...
    BGThread -.->|"bg_results"| BG
    Mate -.->|"MessageBus"| Disp

    class Input,Turn,Lock external
    class Call core
    class Cron,BG,Prep,Pool,Disp,Build mech
    class UPS,PRE,POST,STP mech
    class CronThread,BGThread,Mate external
    class Tasks,Mem,WT,CronJson,Skills persist
...
### s20 的 agent_loop 核心（精简，~100 行）

这是 s20 的代码骨架总览。把 19 个零件装回同一个循环后，整个 harness 的核心就这么多——其余外围都是各零件的实现细节，散落在各自的章节里。
