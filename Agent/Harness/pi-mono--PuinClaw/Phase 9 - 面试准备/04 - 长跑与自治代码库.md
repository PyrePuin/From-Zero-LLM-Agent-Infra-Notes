...
> [!note]
> 这一节回答两个问题:
> **长跑(long-running)** —— 怎么让 agent 持续跑数小时甚至数天而不崩?
> **自治代码库(autonomous codebase)** —— 怎么让 agent 长期驻留、自动管理一个代码库?
>
> 对照两种实现:**pi-mom 的简化做法**(while 循环 + 3 个工具回调)vs **OpenAI Codex 的真实做法**(durable project memory + 4 个 markdown 文件)。
...
## 2. 简化实现:pi-mom goal runner

> PuinClaw 用的就是 pi-mono 的 mom 包(`packages/mom/src/goal/runner.ts`),是一个**最小可行**的长跑实现。

### 2.1 核心机制

**一个 `while (true)` 主循环 + 3 个控制工具回调 + 双层 cap + Resume 三件套**:
...
### 3.2 核心思想:durable project memory
...
### 3.5 Codex app 配套基础设施

| 设施 | 作用 | pi-mom 有没有 |
|---|---|---|
| **Git worktrees** | 隔离每次运行,保持 diff 可 review,减少 thrash | ❌(只在 workspace 一个目录跑) |
...
> [!note]
> 两者不互斥,而是**层级关系**:Codex 的 4 markdown 套路可以**跑在** pi-mom 的 while 循环里——agent 通过工具读/写这 4 个文件,runner 提供 turn/cost/cap/Resume 机制。PuinClaw 演进的下一步就是把这层加上。

## 5. Codex 多了 pi-mom 没做的事(演进清单)

PuinClaw 要从"pi-mom 简化版"走向"Codex 完整版",需要补的:

| 补丁 | 价值 | 难度 |
|---|---|---|
| **4 markdown 套路** | 让 agent 有外部记忆,防目标漂移 | 低(改 System Prompt + 加 read/write 工具) |
| **强制验证工具** | 提供 `verify(criteria, command)` 工具,失败禁止 mark_complete | 中(需要解析验证结果) |
| **Plan mode** | 跑前生成可 review 的步骤序列,人审后再跑 | 中(需要交互式批准) |
| **Git worktree 隔离** | 每次跑用独立 worktree,diff 干净可 review | 低(`git worktree add`) |
| **多 task 并发** | 同一 workspace 多个 goal 独立 state + 独立 transcript | 高(资源隔离 + 调度) |
| **事件驱动入口** | GitHub webhook → 自动开 goal | 高(需要常驻 + 鉴权) |

## 6. 自治代码库:更激进的演进

> 长跑的下一步:**agent 不只是完成一个 goal,而是长期驻留、持续管理一个代码库**。
> Anthropic 的《Building a C compiler with Claude》([engineering blog](https://www.anthropic.com/engineering/building-c-compiler))是这个方向的代表实验。
...
关键特征:**事件驱动 + 多 agent + always-on + human-in-the-loop review**。

### 6.3 PuinClaw 演进路线

```
当前(已完成):/goal 单 agent 长跑(pi-mom 简化版)
   ↓
近期:加 Codex 4 markdown 套路 + 强制验证
   ↓
中期:加 GitHub webhook → 自动开 goal
   ↓
长期:Agent Teams(pm2 守护 + always-on + 自动 PR)
```
...
pi-mom 现在只在 workspace 一个目录改,跑坏了**整个 workspace 都受影响**——这是 Codex 比 pi-mom 强的一个具体地方。
...
### Q5: 既然 Codex 都做出来了,为什么 PuinClaw 还要从 pi-mom 起步?

**学习路线的考虑**,不是技术选型。

- pi-mom 是**最小可行**:while + 工具回调 + cap,300 行代码能读懂。学完知道长跑的基本骨架。
- Codex 是**完整产品**:4 markdown + 强制验证 + worktree + Skills + Automations + Plan mode + Parallel threads。一上来就读会迷失。

所以 PuinClaw 的路线是:**先跑通 pi-mom,理解骨架 → 再逐步加 Codex 的招式**(4 markdown → 强制验证 → worktree → ...)。这是渐进式学习,不是非此即彼。

### Q6: 强制验证会不会让 agent 卡在 lint 上无限循环?
...
pi-mom 没有这个机制,所以即便有 turn cap 兜底,agent 也可能在 100 轮内反复跑同一个 lint 错误。这是 pi-mom 下一步要补的。
